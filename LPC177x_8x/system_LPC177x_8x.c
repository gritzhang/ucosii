...
{
#if (CLOCK_SETUP)                       /* Clock Setup                  1     */
  LPC_SC->SCS       = SCS_Val;          //0x00000021
  if (SCS_Val & (1 << 5)) {             /* If Main Oscillator is enabled      */
    while ((LPC_SC->SCS & (1<<6)) == 0);/* Wait for Oscillator to be ready    */
  }

  LPC_SC->CLKSRCSEL = CLKSRCSEL_Val;    /* Select Clock Source for sysclk/PLL0  0x00000001*/

#if (PLL0_SETUP)                        // 1    
  LPC_SC->PLL0CFG   = PLL0CFG_Val;      //0x00000009    
...
#if (PLL1_SETUP)                        //1
  LPC_SC->PLL1CFG   = PLL1CFG_Val;      //0x00000023 £¿£¿£¿£¿FCCO = 384M > 320M
  ...
  LPC_SC->CCLKSEL   = CCLKSEL_Val;      /* Setup Clock Divider          0x00000001|(1<<8)       */
  LPC_SC->USBCLKSEL = USBCLKSEL_Val;    /* Setup USB Clock Divider      0x00000001|(0x02<<8)    */
  LPC_SC->EMCCLKSEL = EMCCLKSEL_Val;    /* EMC Clock Selection          0x00000001              */
  LPC_SC->PCLKSEL   = PCLKSEL_Val;      /* Peripheral Clock Selection   0x00000002              */
  LPC_SC->PCONP     = PCONP_Val;        /* Power Control for Peripherals   0x00000002  timer/counter0     */
  LPC_SC->CLKOUTCFG = CLKOUTCFG_Val;    /* Clock Output Configuration      0x00000100   */
#endif

#if (FLASH_SETUP == 1)                  /* Flash Accelerator Setup        1   */
  LPC_SC->FLASHCFG  = FLASHCFG_Val|0x03A; //0x00005000 |0x03A
#endif
