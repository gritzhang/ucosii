 ...
 *											
 * ----------	-------		------------------------------------------------
 * 12/08/2015	1.9			modify setSUBR() fuctions
...
static uint8_t SUBN_VAR[4];
static uint8_t IP_VAR[4];
...
//	IINCHIP_ISR_ENABLE();
	   ...
	   IINCHIP_ISR_DISABLE();
	   *((vuint8*)IDM_AR0) = (uint8_t)((addr & 0xFF00) >> 8);
	   *((vuint8*)IDM_AR1) = (uint8_t)(addr & 0x00FF);
	   *((vuint8*)IDM_DR)  = data;
//	   IINCHIP_ISR_ENABLE();
...
uint8_t IINCHIP_READ(uint16_t addr)
{
    ...
    SSP_DATA_SETUP_Type SSP_DATA_buffer;
    
    SSP_DATA_buffer.rx_data = &data;
...
//		IINCHIP_SpiSendData(0xFF);

		buf[idx] = IINCHIP_SpiRecvData(0xFF);
...
{
#if (__DEF_IINCHIP_BUS__ == __DEF_IINCHIP_SPI_MODE__)
    GPIO_SetDir(W5100_CS_PORT, W5100_CS_PIN, GPIO_DIRECTION_OUTPUT);
...
{
	IINCHIP_WRITE((GAR0 + 0),addr[3]);
	IINCHIP_WRITE((GAR0 + 1),addr[2]);
	IINCHIP_WRITE((GAR0 + 2),addr[1]);
	IINCHIP_WRITE((GAR0 + 3),addr[0]);
}
...
{
	IINCHIP_WRITE((SHAR0 + 0),addr[5]);
	IINCHIP_WRITE((SHAR0 + 1),addr[4]);
	IINCHIP_WRITE((SHAR0 + 2),addr[3]);
	IINCHIP_WRITE((SHAR0 + 3),addr[2]);
	IINCHIP_WRITE((SHAR0 + 4),addr[1]);
	IINCHIP_WRITE((SHAR0 + 5),addr[0]);
}
...
{
	IINCHIP_WRITE((SIPR0 + 0),addr[3]);
	IINCHIP_WRITE((SIPR0 + 1),addr[2]);
	IINCHIP_WRITE((SIPR0 + 2),addr[1]);
	IINCHIP_WRITE((SIPR0 + 3),addr[0]);
}
...
//		tmp = (uint8_t) SSP_ReceiveData(LPC_SSP1);
//	} 
//    
//	addr[0] = IINCHIP_READ(GAR0);
//    addr[0] = IINCHIP_READ(GAR0+1);
//	addr[1] = IINCHIP_READ(GAR0+2);
//	addr[2] = IINCHIP_READ(GAR0+3);
//	addr[3] = IINCHIP_READ(GAR0+1);
//}
void getGAR(uint8_t * addr)
{
	addr[0] = IINCHIP_READ(GAR0);
...
void setSn_DPORT(SOCKET s, uint16_t * addr)
{
	IINCHIP_WRITE((Sn_DPORT0(s) + 0),*addr>>8);
	IINCHIP_WRITE((Sn_DPORT0(s) + 1),*addr&0xff);
}
...
*/
void write_data(SOCKET s, vuint8 * src, vuint8 * dst, uint16_t len)
{
	uint16_t size;
	uint16_t dst_mask;
	uint8_t * dst_ptr;
...
*/
void read_data(SOCKET s, vuint8 * src, vuint8 * dst, uint16_t len)
{
	uint16_t size;
	uint16_t src_mask;
	uint8_t * src_ptr;
