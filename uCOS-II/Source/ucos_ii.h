...
typedef struct {
    uint8_t   OSEventType;                   /* Type of event control block (see OS_EVENT_TYPE_???)      */
...
extern	OS_EVENT	*DSPSem;
extern	OS_EVENT	*TCPSem;
...
typedef struct {                            /* Event Flag Wait List Node                               */
    ...
    uint8_t         OSFlagNodeWaitType;       /* Type of wait:                                           */
                                            /*      OS_FLAG_WAIT_AND                                   */
                                            /*      OS_FLAG_WAIT_ALL                                   */
                                            /*      OS_FLAG_WAIT_OR                                    */
                                            /*      OS_FLAG_WAIT_ANY                                   */
...
typedef struct {
    uint8_t   OSEventTbl[OS_EVENT_TBL_SIZE];  /* List of tasks waiting for event to occur                */
    uint8_t   OSEventGrp;                     /* Group corresponding to tasks waiting for event to occur */
    uint8_t   OSValue;                        /* Mutex value (0 = used, 1 = available)                   */
    uint8_t   OSOwnerPrio;                    /* Mutex owner's task priority or 0xFF if no owner         */
    uint8_t   OSMutexPIP;                     /* Priority Inheritance Priority or 0xFF if no owner       */
} OS_MUTEX_DATA;
#endif
...
typedef struct os_q {                   /* QUEUE CONTROL BLOCK                                         */
    ...
    uint16_t         OSQSize;             /* Size of queue (maximum number of entries)                   */
    uint16_t         OSQEntries;          /* Current number of entries in the queue                      */
} OS_Q;
...
#if OS_SEM_EN > 0
typedef struct {
    uint16_t  OSCnt;                          /* Semaphore count                                         */
    uint8_t   OSEventTbl[OS_EVENT_TBL_SIZE];  /* List of tasks waiting for event to occur                */
    uint8_t   OSEventGrp;                     /* Group corresponding to tasks waiting for event to occur */
} OS_SEM_DATA;
#endif
...
typedef struct {
    uint32_t  OSFree;                    /* Number of free bytes on the stack                            */
    uint32_t  OSUsed;                    /* Number of bytes used on the stack                            */
} OS_STK_DATA;
#endif
...
typedef struct os_tcb {
    ...
    uint32_t         OSTCBStkSize;       /* Size of task stack (in number of stack elements)             */
    uint16_t         OSTCBOpt;           /* Task options as passed by OSTaskCreateExt()                  */
    uint16_t         OSTCBId;            /* Task ID (0..65535)                                           */
#endif
    ...
    uint8_t          OSTCBPrio;          /* Task priority (0 == highest, 63 == lowest)                   */

    uint8_t          OSTCBX;             /* Bit position in group  corresponding to task priority (0..7) */
...
OS_EXT  uint32_t            OSCtxSwCtr;               /* Counter of number of context switches           */
...
OS_EXT  uint32_t            OSIdleCtrMax;             /* Max. value that idle ctr can take in 1 sec.     */
OS_EXT  uint32_t            OSIdleCtrRun;             /* Val. reached by idle ctr at run time in 1 sec.  */
...
OS_EXT  uint8_t             OSIntExitY;

OS_EXT  uint8_t             OSLockNesting;            /* Multitasking lock nesting level                 */
...
OS_EXT  uint8_t             OSTaskCtr;                       /* Number of tasks created                  */

OS_EXT  volatile  uint32_t  OSIdleCtr;                                 /* Idle counter                   */
...
OS_EXT  volatile  uint32_t  OSTime;                   /* Current value of system time (in ticks)         */
#endif

extern  uint8_t  const      OSMapTbl[];               /* Priority->Bit Mask lookup table                 */
extern  uint8_t  const      OSUnMapTbl[];             /* Priority->Index    lookup table                 */
...
OS_FLAGS      OSFlagAccept(OS_FLAG_GRP *pgrp, OS_FLAGS flags, uint8_t wait_type, uint8_t *err);
#endif

OS_FLAG_GRP  *OSFlagCreate(OS_FLAGS flags, uint8_t *err);

#if OS_FLAG_DEL_EN > 0
OS_FLAG_GRP  *OSFlagDel(OS_FLAG_GRP *pgrp, uint8_t opt, uint8_t *err);
#endif

OS_FLAGS      OSFlagPend(OS_FLAG_GRP *pgrp, OS_FLAGS flags, uint8_t wait_type, uint16_t timeout, uint8_t *err);
OS_FLAGS      OSFlagPost(OS_FLAG_GRP *pgrp, OS_FLAGS flags, uint8_t operation, uint8_t *err);

#if OS_FLAG_QUERY_EN > 0
OS_FLAGS      OSFlagQuery(OS_FLAG_GRP *pgrp, uint8_t *err);
#endif
#endif
...
#if OS_MBOX_DEL_EN > 0
OS_EVENT     *OSMboxDel(OS_EVENT *pevent, uint8_t opt, uint8_t *err);
#endif
...
uint8_t         OSMboxPostOpt(OS_EVENT *pevent, void *msg, uint8_t opt);
#endif

#if OS_MBOX_QUERY_EN > 0
uint8_t         OSMboxQuery(OS_EVENT *pevent, OS_MBOX_DATA *pdata);
#endif
#endif
...
uint8_t         OSMemQuery(OS_MEM *pmem, OS_MEM_DATA *pdata);
#endif
...
uint8_t         OSMutexAccept(OS_EVENT *pevent, uint8_t *err);
#endif

OS_EVENT     *OSMutexCreate(uint8_t prio, uint8_t *err);

#if OS_MUTEX_DEL_EN > 0
OS_EVENT     *OSMutexDel(OS_EVENT *pevent, uint8_t opt, uint8_t *err);
#endif

void          OSMutexPend(OS_EVENT *pevent, uint16_t timeout, uint8_t *err);
uint8_t         OSMutexPost(OS_EVENT *pevent);

#if OS_MUTEX_QUERY_EN > 0
uint8_t         OSMutexQuery(OS_EVENT *pevent, OS_MUTEX_DATA *pdata);
#endif
...
uint8_t         OSQPostFront(OS_EVENT *pevent, void *msg);
#endif

#if OS_Q_POST_OPT_EN > 0
uint8_t         OSQPostOpt(OS_EVENT *pevent, void *msg, uint8_t opt);
#endif
...
uint16_t        OSSemAccept(OS_EVENT *pevent);
#endif

OS_EVENT     *OSSemCreate(uint16_t cnt);

#if OS_SEM_DEL_EN > 0
OS_EVENT     *OSSemDel(OS_EVENT *pevent, uint8_t opt, uint8_t *err);
#endif

void          OSSemPend(OS_EVENT *pevent, uint16_t timeout, uint8_t *err);
uint8_t         OSSemPost(OS_EVENT *pevent);

#if OS_SEM_QUERY_EN > 0
uint8_t         OSSemQuery(OS_EVENT *pevent, OS_SEM_DATA *pdata);
#endif
...
uint8_t         OSTaskChangePrio(uint8_t oldprio, uint8_t newprio);
#endif

#if OS_TASK_CREATE_EN > 0
uint8_t         OSTaskCreate(void (*task)(void *pd), void *pdata, OS_STK *ptos, uint8_t prio);
#endif

#if OS_TASK_CREATE_EXT_EN > 0
uint8_t         OSTaskCreateExt(void  (*task)(void *pd),
                              void   *pdata,
                              OS_STK *ptos,
                              uint8_t   prio,
                              uint16_t  id,
...
#if OS_TASK_CREATE_EXT_EN > 0
uint8_t         OSTaskStkChk(uint8_t prio, OS_STK_DATA *pdata);
#endif

#if OS_TASK_QUERY_EN > 0
uint8_t         OSTaskQuery(uint8_t prio, OS_TCB *pdata);
#endif
...
uint8_t         OSTimeDlyHMSM(uint8_t hours, uint8_t minutes, uint8_t seconds, uint16_t milli);
#endif
...
#if ((OS_Q_EN > 0) && (OS_MAX_QS > 0)) || (OS_MBOX_EN > 0) || (OS_SEM_EN > 0) || (OS_MUTEX_EN > 0)
uint8_t         OS_EventTaskRdy(OS_EVENT *pevent, void *msg, uint8_t msk);
...
uint8_t         OS_TCBInit(uint8_t prio, OS_STK *ptos, OS_STK *pbos, uint16_t id, uint32_t stk_size, void *pext, uint16_t opt);
...
OS_STK       *OSTaskStkInit(void (*task)(void *pd), void *pdata, OS_STK *ptos, uint16_t opt);
